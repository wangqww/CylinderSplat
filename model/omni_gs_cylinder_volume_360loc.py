...
class OmniGaussianCylinderVolume360Loc(BaseModule):
    ...
    def forward(self, batch, split="train", iter=0, iter_end=100000):
        ...
        with torch.no_grad():
            ...
            gaussians_pixel_render = rearrange(gaussians_pixel, "b v hw c -> (b v) hw c")
            ...
            render_pkg_pixel = self.renderer.render(
                gaussians=gaussians_pixel_render,
                c2w=render_c2w,
                fovx=render_fovxs,
                fovy=render_fovys,
                rays_o=None,
            ...
            tmp_pixel_alpha = rearrange(render_pkg_pixel["alpha"], "(b v) vc c h w -> b vc v c h w", b=bs, v=v) # b v vc 1 h w
            ...
            tmp_pixel_alpha = tmp_pixel_alpha * weights[..., None, None, None]
            render_pkg_pixel["alpha"] = tmp_pixel_alpha.sum(dim=2, keepdim=False) # b v 1 h w
        ...
        render_pkg_fuse = {}
        volume_weights = render_pkg_volume['alpha']
        render_pkg_fuse["image"] = render_pkg_pixel["image"] * (1 - volume_weights) + render_pkg_volume["image"]
        render_pkg_fuse["depth"] = render_pkg_pixel["depth"] * (1 - volume_weights) + render_pkg_volume["depth"]
        render_pkg_fuse['alpha'] = render_pkg_pixel['alpha'] * (1 - volume_weights) + render_pkg_volume['alpha']
        ...
        if self.loss_args.weight_volume_loss > 0 and iter < iter_end:
            target = torch.ones_like(volume_weights, device=volume_weights.device)
            volume_loss = F.mse_loss(volume_weights * mask_dptm.unsqueeze(2), target * mask_dptm.unsqueeze(2))
