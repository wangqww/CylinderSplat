...
class OmniGaussianCylinderAllUniFuse(BaseModule):
    ...
    def voxelizaton_with_fusion(self, img_feat, pts3d, voxel_size=0.2, conf=None):
        ...
        # pts3d: N, 3
        N, C = img_feat.shape
        pts3d_flatten = pts3d.view(-1, 3)

        voxel_indices = (pts3d_flatten / voxel_size).round().int()  # [N, 3]
        ...
        anchor_feats_flat = img_feat.view(-1, C)  # [N, C]
        ...
        weighted_pts = pts3d_flatten * weights  # [N, 3]
        weighted_feats = anchor_feats_flat * weights  # [N, C] - 修复这里，移除错误的squeeze(1)
    ...
    def forward(self, batch, split="train", iter=0, iter_end=100000):
        ...
        if self.loss_args.weight_volume_loss > 0:
            volume_loss = (- render_pkg_fuse["alpha"] * torch.log(render_pkg_fuse["alpha"] + 1e-8)
                           - (1 - render_pkg_fuse["alpha"]) * torch.log(1 - render_pkg_fuse["alpha"] + 1e-8)).mean()
