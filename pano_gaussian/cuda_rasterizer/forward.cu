...
// Forward version of 2D covariance matrix computation
__device__ float3 computePanoramaCov2D(const float3& mean, int pano_W, int pano_H, const float* cov3D, const float* viewmatrix)
{
	// The following models the steps outlined by equations 29
	// and 31 in "EWA Splatting" (Zwicker et al., 2002). 
	// Additionally considers aspect / scaling of viewport.
	...
	float W_div_2pi = pano_W * 0.5f * M_1_PIf32;
	float H_div_pi = pano_H * M_1_PIf32;

	glm::mat3 J = glm::mat3(
		W_div_2pi * t.z * x2z2_inv, 0.0f, -W_div_2pi * t.x * x2z2_inv,
		-H_div_pi * t.x * t.y * x2z2_sqrt_inv * x2y2z2_inv, H_div_pi * x2z2_sqrt * x2y2z2_inv, -H_div_pi * t.z * t.y * x2z2_sqrt_inv * x2y2z2_inv,
		0, 0, 0);
	...
	bool prefiltered)
{
	...
	if (W == 2 * H) // Panorama
	{
		p_proj = { 2.0f * p_view.x - 1.0f, 2.0f * p_view.y - 1.0f, 2.0f * (p_view.z - 0.2f) / (10000.0f - 0.2f) - 1.0f };
	}
	else
	{
	...
	if (W == 2 * H)
	{
		cov = computePanoramaCov2D(p_orig, W, H, cov3D, viewmatrix);
	}
	else
	{
