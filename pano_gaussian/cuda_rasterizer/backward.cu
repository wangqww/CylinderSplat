	...
	glm::vec4* dL_drot)
{
	...
	if (width == 2 * height)
	{
		...
		float x2y2z2_sqrt_inv = 1.0f / sqrtf(t.x * t.x + t.y * t.y + t.z * t.z + 0.0000001f);
		// Compute loss gradient w.r.t. 3D means due to gradients of depth
		// from rendering procedure
		float dL_dtx2 = (t.x * x2y2z2_sqrt_inv) * dL_ddepth[idx];
		float dL_dty2 = (t.y * x2y2z2_sqrt_inv) * dL_ddepth[idx];
		float dL_dtz2 = (t.z * x2y2z2_sqrt_inv) * dL_ddepth[idx];
		float3 dL_dmean3D2 = transformVec4x3Transpose({ dL_dtx2, dL_dty2, dL_dtz2 }, view);
		glm::vec3 dL_dmean2(dL_dmean3D2.x, dL_dmean3D2.y, dL_dmean3D2.z);
