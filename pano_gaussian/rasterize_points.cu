	...
	)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		confidence.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(rendered, out_color, out_feature_map, out_confidence, out_alpha, out_depth, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dconfidence = torch::zeros({P, 1}, means3D.options());
  ...
  {  
	  ...
	  dL_dconfidence.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dfeatures, dL_dconfidence, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
